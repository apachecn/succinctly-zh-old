...
碎片遵循与活动相同的生命周期模式。您可以定义自定义 `onCreate()` ， `onPause()` ， `onResume()` ，以及我们在本书前面讨论过的所有其他活动生命周期回调方法在 `Fragment` 子类中。当在主机活动上调用其中一个生命周期方法时，它会将其传递给它包含的任何片段。这是使片段如此模块化的一部分 - 它们就像活动一样，但完全是独立的。
