...
这个例子已经非常复杂，我故意省略了前一个和下一个按钮之间链接呈现的解释。 [完整实施](https://github.com/fdietz/recipes-with-angular-js-examples/tree/master/chapter8/recipe2) 在线，但您可以进行调查。
...
您希望在用户成功提交表单后显示闪存确认消息。

### 解决方案

在像 Ruby on Rails 这样的 Web 框架中，表单提交将导致重定向闪存确认消息，依赖于浏览器会话。对于我们的客户端方法，我们绑定路由更改并管理闪存消息队列。
...
请注意，闪存消息（就像导航一样）始终显示，但有条件地隐藏，具体取决于是否有可用的闪存消息。
...
在模板中，我们使用`ng-show`使用`flash.getMessage()`通过闪存消息隐藏 div 元素。
...
拦截器使用工厂方法实现，并将自身附加到 AJAX 响应的 promise 函数，以隐藏成功或失败的微调器。请注意，失败时，我们使用 [$ q](http://docs.angularjs.org/api/ng.$q) 服务的`reject`功能，Angular 的承诺/延期实施。
