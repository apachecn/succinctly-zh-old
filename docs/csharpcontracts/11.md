...
让我们假设钢铁制造商需要尽量减少切割过程产生的切割量。为此，他们使用计算来计算要切割的钢的体积，并根据切削系数计算可以使用的钢的数量。完美切割意味着如果钢的体积为 10 m &lt;sup class="calibre37"&gt;3 ，那么所得到的钢材切割量也将等于 10 m &lt;sup class="calibre37"&gt;3 。因此，所有的钢都被消耗掉，导致零切。
...
但是，如果 volumeSteel 和 CutSteel 的总和不等于 volumeSteel ，则该因子导致切割过程产生切割。运行控制台应用程序，钢卷体积值为 4 m &lt;sup class="calibre37"&gt;3 ，因子为 3 会导致切割。
